...
namespace AccountingSystem.Web
{
    public class DetailController : Controller
    {
        ...
        public dynamic SumBy([FromBody] Condition condition)
        {
            ...
            var target = new List<KeyValuePair<AccountingSubject, List<KeyValuePair<int, double>>>>();

            var beginYear = condition.TradingDayBegin.Value.Year;
            var endYear = condition.TradingDayEnd.Value.Year;
            for (int i = beginYear; i <= endYear; i++)
            {
                years.Add(i);
                var keyValuePairs = detailRepository.GroupBy(new DateTime(i, 1, 1), new DateTime(i, 12, 31));
                foreach(var keyValuePair in keyValuePairs)
                {
                    ...
                    if (!target.Any(item => item.Key.Code == accountingSubjectCode))
                        target.Add(new KeyValuePair<AccountingSubject, List<KeyValuePair<int, double>>>(accountingSubject, new List<KeyValuePair<int, double>>()));

                    var yearAmountPair = target.FirstOrDefault(item => item.Key.Code == accountingSubjectCode);
                    yearAmountPair.Value.Add(new KeyValuePair<int, double>(i, amount));
                }
            }

            target = target.OrderBy(item => item.Key.Code).ToList();
            return new { years, target };
        } 
