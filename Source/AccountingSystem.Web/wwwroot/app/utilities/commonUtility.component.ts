...
export class HttpErrorResponseUtility {
    static Notify(httpErrorResponse: any) {
        ...
        else if (httpErrorResponse.status === 500 && httpErrorResponse.message) {
            $.notify({ icon: "tim-icons icon-bell-55", message: httpErrorResponse.message }, { type: 'warning', delay: 0, placement: { from: 'top', align: 'right' } });
        }
        else {
