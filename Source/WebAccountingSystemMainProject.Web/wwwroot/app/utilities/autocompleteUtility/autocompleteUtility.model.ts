...
export class AutocompleteUtilityModel {
    /**
     * 輸入框內容
     */
    inputValue: string = "";
    ...
    set menu(dictionary: Dictionary) {
        ...
        for (var key in this._menu) {
            const $li = $('<li class="ui-menu-item ui-widget-content">').html(`<a key="${key}" class="ui-menu-item-wrapper">${this._menu[key]}</a>`);
            $lis.push($li);
        }
    ...
    displayMenu() {
        if (Object.keys(this._menu).length > 0) {
            ...
            this.$element.on('keydown', function (event: any) {
                if (this.isHookKey(event)) {
                    if (event.which == 13) {
                        const $ele = this.$element;
                        this.onSelected({ target: $ele.find(`li:nth-child(${this.currentIndex + 1}) a`)[0] });
                        this.hideMenu($ele);
                        return;
